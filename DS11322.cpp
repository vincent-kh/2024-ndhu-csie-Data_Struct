...
public:
	...
	WeightedGraphVertex<V, E>** getend()
	{
		return end;
	}
    ...
    void print()
    {
        std::cout<<end[0]->getData()<<"-"<<end[1]->getData()<<"("<<this->getData()<<") ";
    }
...
public:
    ...
    {
        edge->sort();
        ListNode<WeightedGraphEdge<V, E> *> *cur = edge->head;
        while(cur != NULL)
        {
            cur->getData()->print();
            std::cout<<std::endl;
    ...
    void vertex_print()
    {
        ListNode<WeightedGraphVertex<V, E> *> *cur = vertex->head;
        while(cur != NULL)
        {
            std::cout<<cur->getData()->getData()<<' ';
            cur = cur->getNext();
        }
        std::cout<<std::endl;
    }
	...
	WeightedGraph *minimumSpanningTree(WeightedGraphVertex<V, E> *v)
	{
        ...
        edge->sort();
        int sets[vertexCount];
        for(int i = 0;i < vertexCount;i++)  {sets[i] = i;}
        ListNode<WeightedGraphEdge<V, E> *> *cur = edge->head;
        while(cur != NULL)
        {
            WeightedGraphVertex<V, E> *v1 = cur->getData()->getend()[0];
            WeightedGraphVertex<V, E> *v2 = cur->getData()->getend()[1];
            if(v1!=v2 && sets[v1->getData()-'a'] != sets[v2->getData()-'a'])
            {
				tree->addLink((*tree->vertex)[v1->getData()-'a'].getData(), (*tree->vertex)[v2->getData()-'a'].getData(), cur->getData()->getData());
                int temp = sets[v2->getData()-'a'];
                for(int i = 0;i < vertexCount;i++)
                {
                    if(sets[i] == temp)
                        sets[i] = sets[v1->getData()-'a'];
                }
            }
            cur = cur->getNext();
        }
